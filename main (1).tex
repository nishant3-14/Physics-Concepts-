...
\title{\Huge Down The Rabbit Hole of Physics\\[0.5em]
\Large A Curious Journey into the Mathematics of Relativity}
...
Mathematics and physics are often taught as collections of formulas, procedures, and 
abstract definitions to be memorised. Students learn to manipulate symbols long before 
they are shown what those symbols mean geometrically or physically. As a result, ideas 
that are often simple at their core can begin to feel mysterious.

\medskip

This book approaches these ideas differently.

\medskip

Like Alice following the rabbit down the rabbit hole, the reader is invited to begin with 
something familiar and simple, only to discover that it leads into a much deeper world. 
A vector is not merely an arrow or a list of numbers. Coordinates are not reality itself. 
A matrix is not just an array of entries. Beneath these familiar objects lies a geometric 
language for describing motion, space, measurement, and eventually spacetime.

\medskip

The goal of this text is not merely to teach linear algebra, tensors, or relativity as 
computational subjects, but to build an intuitive geometric understanding of the structures 
behind them. We will return often to simple pictures --- walking on a hill, changing 
coordinate systems, stretching space, measuring directions --- and use them to uncover 
the mathematics beneath.

\medskip

The philosophy of this book is that mathematics should arise naturally from geometry. 
Rather than beginning with formal definitions and adding interpretation later, we will 
often begin with intuition and then formalise it carefully.

\medskip

A recurring theme is the distinction between an object and its description. A vector is 
not its coordinates. A transformation is not the matrix used to represent it. A geometric 
or physical statement should not depend on the coordinate system chosen to describe it. 
This idea leads naturally toward vectors, covectors, tensors, metrics, and eventually 
spacetime itself.

\medskip

This text grew out of personal study in linear algebra, geometry, and relativity, and 
from a desire to understand these subjects conceptually rather than purely computationally. 
Many of the explanations arose from asking a simple question:
...
This book is an invitation to follow that rabbit hole slowly --- curious, patient, and 
willing to let familiar ideas become strange before they become clear.
...
and one for vertical height. This is what we call $\mathbb{R}^2$.
...
Even though we cannot easily picture higher dimensions, the idea is the same: 
...
if a vector has coordinates $\vec{v}'$ in the new basis, then its coordinates in the 
standard basis are:
